...
public class Cafe_Baixado_ServiceTest {
	...
	@Autowired
	private Cafe_Baixado_Service cafe_baixadoService; //Chamo para testá-la. "Obtém" uma instância de Cafe_Baixado_Service por meio da anotação @Autowired
	
	private static final int Lote = 12;
	...
	public void setUp() throws Exception{ //qualquer inteiro que ele receber, retornará uma instância de Cafe_Baixado |
		...
		BDDMockito.given(this.cafe_baixadoRepository.save(Mockito.any(Cafe_Baixado.class))).willReturn(new Cafe_Baixado());//Passa qualquer objeto do tipo Cafe_Baixado, retornará um novo Cafe_Baixado
	}
	
	@Test
	public void testBuscarCafeBaixadoPorLote() {
		Optional<Cafe_Baixado> cafe_baixado = this.cafe_baixadoService.buscarPorLote(Lote);
	...
	public void testPersistirCafe_Baixado() {
		Cafe_Baixado cafe_baixado = this.cafe_baixadoService.persistir(new Cafe_Baixado());//Quando eu chamar o persistir, o Mockito me retornará uma nova instância de Cafe_Baixado
